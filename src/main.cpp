...
/**
 * Convert a CImg instance into a char array as an RGB image.
 *
 * @param image     image as CImg<unsigned char>
 * @return in       RGB image as 1-D char array of length (w * h * 3)
 * */
 ...
 * @param in    RGB image as a char array, whose length is expected to be 3 * N
 * @param out   char array to hold the converted grayscale image, whose length is expected to be N
 * */
...
int main() {
    ...
    unsigned char * grayscale[5];  // each grayscale[i] is a grayscale image as a char array of length (w * h)
    unsigned char * in[5];
    bool * group1[5];  // whether belongs to the background
    bool * group2[5];  // whether belongs to the obstruction
    unsigned char * gradient[3];
    pair<vector<Point2f>, vector<Point2f>> edgeFlowPairs[5];

    // ===========================================
    // Convert Images to Grayscale Images as Array
    // ===========================================
    for (int i = 0; i < 5; i++) {
        grayscale[i] = new unsigned char[N / 3];
#if GRAYSCALE == 1
        // if the input images are grayscale already, simply copy pixels from CImg data structure to char array
...
#else
        // otherwise (the image images are RGB), get grayscale images by averaging channel intensities on pixels
        in[i] = toRGB(images[i]);
	...
	int SPARSE_SIZE = edgeFlowPairs[0].first.size();  // number of edge points
	bool * pointGroup = new bool[SPARSE_SIZE];  // whether an edge pixel belongs to the background
    bool * sparseMap = new bool[N / 3];  // whether a pixel is an edge pixel
    unsigned char * lkViz = new unsigned char[N / 3];  // to hold image to draw
    glm::vec2 * pointDiffs = new glm::vec2[SPARSE_SIZE];  // movement/displacement for each edge pixel
	RansacSeparator * ransacSeparator = new RansacSeparator(SPARSE_SIZE);
	pair<glm::vec2,glm::vec2> groupVectors[4];  // groupVectors[i]: pair of vec2, the first one is
                                                // the average background movement, the second is for foreground
    ...
    for (int j = 0; j < 4; j++) {
        ...
        // ======================================
        // Background/Obstruction Edge Seperation
        // ======================================
        printf("\nSeparating...\n=============================================\n");
        group1[j] = new bool[N / 3];  // whether belongs to the background
        group2[j] = new bool[N / 3];  // whether belongs to the obstruction
